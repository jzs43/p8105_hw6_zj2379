...
```
    ...
    ##  5 2          36      52  3629   154      65 1        40.3 0             11
    ##  6 1          34      63  3175   143      25 1        41.9 0             13
    ##  7 2          35      51  3827   130      45 1        41.3 0             12
    ##  8 2          34      49  3317   142      35 1        40.4 0             12
...
cv_df = 
  ...
  mutate(
    ...
    reg_2  = map(train, \(df) lm(bwt ~ blength+ gaweeks, data = birthweight)),
    reg_3  = map(train, \(df)lm(bwt ~ bhead+ blength+ babysex+ bhead * blength + bhead * babysex + blength * babysex, data = birthweight))) |> 
  mutate(
    reg_1 = map2_dbl(reg_1, test, \(mod, df) rmse(model = mod, data = df)),
    reg_2 = map2_dbl(reg_2, test, \(mod, df) rmse(model = mod, data = df)),
    reg_3 = map2_dbl(reg_3, test, \(mod, df) rmse(model = mod, data = df)))
```

    ## Warning: There were 100 warnings in `mutate()`.
    ## The first warning was:
    ## ℹ In argument: `reg_1 = map2_dbl(reg_1, test, function(mod, df) rmse(model =
    ##   mod, data = df))`.
